@php
    ...
    function DT_FlightDays($flight_days)
    {
      ...
      for ($i=0; $i<7; $i++) {
        if ($flight_days & pow(2, $i)) {
          $days[]=substr(jddayofweek($i, 2), 0, -1);
        }
      }
  ...
  if (!function_exists('DT_FlightType')) {
    function DT_FlightType($flight_type, $type = null)
    {
      ...
      if ($type === 'badge') {
        $flight_type = '<span class="badge badge-phoenix badge-phoenix-secondary">'.$flight_type.'</span>';
      } elseif ($type === 'button') {
        $flight_type = '<span class="btn btn-subtle-secondary">'.$flight_type.'</span>';
      }
  ...
  if (!function_exists('DT_RouteCode')) {
    function DT_RouteCode($route_code, $type = null)
    {
      ...
      if ($type === 'badge') {
        $route_code = '<span class="badge badge-phoenix badge-phoenix-warning">'.$route_code.'</span>';
      } elseif ($type === 'button') {
        $route_code = '<span class="btn btn-subtle-warning">'.$route_code.'</span>';
      }
  ...
  if (!function_exists('DT_RouteLeg')) {
    function DT_RouteLeg($route_leg, $type = null)
    {
      ...
      if ($type === 'badge') {
        $route_leg = '<span class="badge badge-phoenix badge-phoenix-warning"> Leg# '.$route_leg.'</span>';
      } elseif ($type === 'button') {
        $route_leg = '<span class="btn btn-subtle-warning"> Leg# '.$route_leg.'</span>';
      }
  ...
  if (!function_exists('DT_PirepState')) {
    function DT_PirepState($pirep, $type = 'badge')
    {
      ...
      if ($type === 'bg') {
        ...
        $result = '<span class="badge badge-phoenix badge-phoenix-'.$color.'">'.PirepState::label($state).'</span>';
      } else {
  ...
  if (!function_exists('DT_PirepStatus')) {
    function DT_PirepStatus($pirep, $type = 'badge')
    {
      ...
      if ($type === 'bg') {
        ...
        $result = '<span class="badge badge-phoenix badge-phoenix-'.$color.'">'.PirepStatus::label($status).'</span>';
      } else {
  ...
  if (!function_exists('DT_UserState')) {
    function DT_UserState($user, $type = 'badge')
    {
      ...
      if ($type === 'bg') {
        ...
        $result = '<span class="badge badge-phoenix badge-phoenix-'.$color.'" title="'.$title.'">'.UserState::label($state).'</span>';
      } else {
