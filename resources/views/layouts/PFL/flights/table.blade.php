      ...
      <tr>
        <td>
          <a href="{{ route('frontend.flights.show', [$flight->id]) }}">
            ...
            @if(filled($flight->route_code) && in_array($flight->route_code, $tour_codes))
              <span class="badge badge-phoenix badge-phoenix-warning mx-1 px-1">Tour Flight</span>
            @endif
          </a>
        </td>
        @if(Theme::getSetting('flights_codeleg'))
        ...
        <td>
          @if(Theme::getSetting('flights_flags'))
            <img style="height:25px" title="{{ strtoupper(optional($flight->dpt_airport)->country) }}"
        ...
        <td>
          @if(Theme::getSetting('flights_flags'))
            <img style="height:25px" title="{{ strtoupper(optional($flight->arr_airport)->country) }}"
        ...
        <td class="text-end">
          @if(!setting('pilots.only_flights_from_current') || $flight->dpt_airport_id == optional($user->current_airport)->icao)
            ...
            @if(setting('bids.allow_multiple_bids') === true || setting('bids.allow_multiple_bids') === false && count($saved) === 0)
              <button class="btn btn-sm m-0 mx-1 save_flight {{ isset($saved[$flight->id]) ? 'btn-subtle-danger':'btn-subtle-success' }}"
                      x-id="{{ $flight->id }}"
                      x-saved-class="btn-subtle-danger"
            ...
            @if($simbrief !== false && $flight->simbrief && $flight->simbrief->user_id === $user->id)
              <a href="{{ route('frontend.simbrief.briefing', $flight->simbrief->id) }}" class="btn btn-sm m-0 mx-1 btn-subtle-secondary">
            ...
            @elseif($simbrief !== false && ($simbrief_bids === false || $simbrief_bids === true && isset($saved[$flight->id])))
              ...
              @endphp
              <a href="{{ route('frontend.simbrief.generate') }}?flight_id={{ $flight->id }}@if($aircraft_id)&aircraft_id={{ $aircraft_id }} @endif" class="btn btn-sm m-0 mx-1 {{ isset($saved[$flight->id]) ? 'btn-subtle-success':'btn-subtle-primary' }}">
                <i class="fas fa-file-pdf" title="Generate SimBrief OFP"></i>
              </a>
            @endif
            {{-- vmsAcars Load --}}
            @if($acars_plugin && isset($saved[$flight->id]))
              <a href="vmsacars:bid/{{ $saved[$flight->id] }}" class="btn btn-sm m-0 mx-1 btn-subtle-warning">
                <i class="fas fa-file-download" title="Load in Acars"></i>
              </a>
            @elseif($acars_plugin)
              <a href="vmsacars:flight/{{ $flight->id }}" class="btn btn-sm m-0 mx-1 btn-subtle-warning">
                <i class="fas fa-file-download" title="Load in Acars"></i>
              </a>
            @endif
            @if(Theme::getSetting('pireps_manual'))
              <a href="{{ route('frontend.pireps.create') }}?flight_id={{ $flight->id }}" class="btn btn-sm btn-subtle-info m-0 mx-1">
