    ...
    <div class="card mb-2">
        ...
        <div class="card-footer p-1 text-center">
            <div class="row">
                {{-- Badges --}}
                <div class="col text-start">
                    {!! DT_FlightType($flight->flight_type, 'badge') !!}
                    {!! DT_RouteCode($flight->route_code, 'badge') !!}
                    {!! DT_RouteLeg($flight->route_leg, 'badge') !!}
                </div>
                {{-- Buttons --}}
                <div class="col text-end">
                    {{-- }}
          ...
          {{-- }}
                    {{-- Bid Add/Remove --}}
                    <button class="btn btn-sm save_flight {{ isset($saved[$flight->id]) ? 'btn-subtle-danger' : 'btn-subtle-success' }}" x-id="{{ $flight->id }}" x-saved-class="btn-subtle-danger"
                        type="button">
                        {{ isset($saved[$flight->id]) ? __('disposable.bid_rem') : __('disposable.bid_add') }}
                    </button>
                    @if (!setting('pilots.only_flights_from_current') || $flight->dpt_airport_id == optional($user->current_airport)->icao)
                        ...
                        @if ($simbrief !== false && $flight->simbrief && $flight->simbrief->user_id === $user->id)
                            <a href="{{ route('frontend.simbrief.briefing', $flight->simbrief->id) }}" class="btn btn-sm btn-subtle-secondary">@lang('disposable.sb_view')</a>
                        @elseif($simbrief !== false && ($simbrief_bids === false || ($simbrief_bids === true && isset($saved[$flight->id]))))
                            ...
                            <a href="{{ route('frontend.simbrief.generate') }}?flight_id={{ $flight->id }}@if ($aircraft_id) &aircraft_id={{ $aircraft_id }} @endif"
                                class="btn btn-sm btn-subtle-primary ">@lang('disposable.sb_generate')</a>
                        @endif
                        {{-- vmsACARS --}}
                        @if ($acars_plugin && isset($saved[$flight->id]))
                            <a href="vmsacars:bid/{{ $saved[$flight->id] }}" class="btn btn-sm btn-subtle-warning">@lang('disposable.load_acars')</a>
                        @elseif($acars_plugin)
                            <a href="vmsacars:flight/{{ $flight->id }}" class="btn btn-sm btn-subtle-warning">@lang('disposable.load_acars')</a>
                        @endif
                        {{-- Manual Pirep --}}
